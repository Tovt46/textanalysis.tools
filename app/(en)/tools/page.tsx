...
export default function ToolsPage(){
  return <main className="tool-page">
    ...
    <section className="tools-directory" aria-labelledby="available-tools"><div className="directory-heading"><p className="section-number">AVAILABLE NOW</p><h2 id="available-tools">Choose the analysis that matches the question</h2></div><div className="directory-feature-grid"><Link className="tool-directory-card contract-feature-card" href="/tools/text-contract"><div><span>NEMOTRON · GUARDED REWRITE</span><h3>TextContract Agent</h3><p>Turn one source and one brief into approved rules, then let the agent write, check, and repair its own output once.</p></div><strong>Open agent →</strong></Link><Link className="tool-directory-card evidence-feature-card" href="/tools/evidence-workspace"><div><span>WEBMCP · HUMAN-IN-THE-LOOP</span><h3>Claim-Aware Version Review</h3><p>Map rewritten blocks, compare commercial claims, prioritize verification risks, and let ChatGPT propose exact patches while you retain final approval.</p></div><strong>Open workspace →</strong></Link></div><div className="live-tools-grid">{liveTools.map(([href,name,label,copy,theme])=><Link key={name} className={`tool-directory-card ${theme}`} href={href}><div><span>{label} · LIVE</span><h3>{name}</h3><p>{copy}</p></div><strong>Open tool →</strong></Link>)}</div></section>
