...
const description="How Text Analysis Tools handles pasted text, TextContract model requests, public URLs, API requests, local workflows, browser storage, logs, and optional analytics.";
...
const handlingRows=[
  ...
  ["TextContract Agent","Nebius Token Factory; Tavily only for live rules","The source, brief, and candidate are sent to Nemotron for the current stateless step. Up to three explicitly current claims can be sent to Tavily as search queries. This tab keeps the workspace in sessionStorage."],
    ...
    <article>
      ...
      <div className="article-layout">
        ...
        <div className="article-body api-docs-body">
          ...
          <section id="text-contract">
            <p className="section-number">03</p><h2>TextContract model processing</h2>
            <p>TextContract cannot perform a guarded rewrite without reading the source. After you choose Build contract, the source and brief are sent to NVIDIA Nemotron through Nebius Token Factory. Approved rules, the source, and the brief are sent again to create a draft; the source and candidate are sent for semantic evaluation. A detected violation can trigger one repair and one re-evaluation.</p>
            <p>Tavily receives a focused search query only when an approved rule is explicitly marked as requiring live evidence. A run is limited to three live rules and three returned sources per rule. Official domains supplied in the form are used as source filters. Text Analysis Tools does not send every claim to Tavily and does not use it as a general fact checker.</p>
            <p>The application does not save a server-side TextContract workspace. The current browser tab stores source, brief, contract, attempts, and evaluation in <code>sessionStorage</code> so a refresh can restore the run. Closing the tab removes normal session storage; browser restore behavior can vary. Nebius and Tavily process their requests under their own privacy and retention terms.</p>
            <div className="article-callout subtle"><b>Do not submit secrets or unauthorized personal data</b><p>Use TextContract only for content you have the right to process. API keys stay on the server, but the document itself must leave the browser for the model-backed workflow.</p></div>
          </section>

          <section id="api">
            <p className="section-number">04</p><h2>Public API processing</h2>
          ...
          <section id="urls">
            <p className="section-number">05</p><h2>Fetching public URLs</h2>
          ...
          <section id="local">
            <p className="section-number">06</p><h2>CLI and local MCP workflows</h2>
          ...
          <section id="operations">
            <p className="section-number">07</p><h2>Quota state and privacy-safe operational logs</h2>
          ...
          <section id="analytics">
            <p className="section-number">08</p><h2>Optional analytics</h2>
          ...
          <section id="control">
            <p className="section-number">09</p><h2>Retention, control, and safe use</h2>
            <ul>
            ...
            <p>This description reflects the open-source implementation as of September 3, 2026. If the data flow changes, the implementation and this page should change together.</p>
          </section>
