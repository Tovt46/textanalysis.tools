    ...
    <article>
      ...
      <div className="article-layout"><aside className="article-toc" aria-label="On this page"><b>On this page</b><a href="#quickstart">Quickstart</a><a href="#analyze">Analyze endpoint</a><a href="#focused">Focused endpoints</a><a href="#compare">Compare endpoint</a><a href="#weighted">Weight and similarity</a><a href="#response">Response fields</a><a href="#limits">Limits and safety</a><a href="#discovery">Agent discovery</a></aside>
        <div className="article-body api-docs-body">
          ...
          <section id="analyze"><p className="section-number">02</p><h2>Analyze text or a webpage</h2><p><code>POST /api/v1/analyze</code> accepts raw text, HTML, or a public HTTP(S) URL. Set <code>sourceType</code> to <code>text</code> or <code>url</code>.</p><div className="api-fields"><div><b>source</b><span>required string</span><p>The text, HTML, or URL to analyze.</p></div><div><b>language</b><span>auto · en · ru · uk · es</span><p>Defaults to automatic detection.</p></div><div><b>focus</b><span>string or string[]</span><p>Up to 100 non-empty, analyzable phrases to count and report per 1,000 words. Each phrase may contain at most 200 characters; separate string values with commas.</p></div><div><b>top</b><span>5–100</span><p>Maximum number of word and bigram rows.</p></div><div><b>tolerance</b><span>1.2–4</span><p>Threshold for above/below-model diagnostics.</p></div><div><b>keepStopwords</b><span>boolean</span><p>Keep common language stop words when true.</p></div></div></section>
          <section id="focused"><p className="section-number">03</p><h2>Use a focused frequency, vector, or density endpoint</h2><p><code>POST /api/v1/word-frequency</code> returns vocabulary rows with raw count and normalized rates. <code>POST /api/v1/keyword-density</code> returns unigram, bigram, trigram, and exact tracked-phrase tables. <code>POST /api/v1/ngram-analyzer</code> returns n-gram tables for an explicit phrase length.</p><p><code>POST /api/v1/bag-of-words</code> builds term vectors with token count, frequency, and normalized rates.</p><pre className="api-code"><code>{densityExample}</code></pre><pre className="api-code"><code>{bagOfWordsExample}</code></pre><p>The density endpoint accepts <code>trackedKeywords</code> as a comma, semicolon, or newline-separated string, while n-gram analysis accepts <code>ngramSize</code> (1-10). Set <code>limit</code> from 1 to 5,000 and <code>offset</code> from 0 to 250,000. Follow <code>nextOffset</code> until it is null to retrieve the complete ordered table.</p><pre className="api-code"><code>{ngramExample}</code></pre></section>
          <section id="compare"><p className="section-number">04</p><h2>Compare versions A and B</h2><p><code>POST /api/v1/compare</code> accepts two complete analysis inputs and returns both results plus metric and word-frequency differences. To inspect the same result manually, open the <Link href="/tools/text-analysis-comparison">Text Analysis Comparison tool</Link>.</p><pre className="api-code"><code>{compareExample}</code></pre><p><code>wordChanges</code> is ordered by the largest absolute change in share, useful for revision and QA workflows. Apply the same <code>limit</code> and <code>offset</code> to the ordered word and bigram change tables, then follow <code>nextOffset</code> while <code>hasMore</code> is true.</p></section>
          <section id="weighted"><p className="section-number">05</p><h2>Weighting and similarity checks</h2><p><code>POST /api/v1/tf-idf</code> computes TF-IDF tables for 2–10 documents and returns global inverse-document frequencies. <code>POST /api/v1/similarity</code> returns cosine similarity and contribution rows for BoW or TF-IDF vectors.</p><p>For these operations, <code>top</code> controls per-document or contribution rows, while <code>limit</code> and <code>offset</code> page the supporting <code>idfTable</code>. Follow <code>nextIdfOffset</code> while <code>hasMoreIdfRows</code> is true. A BoW similarity response has no IDF table, but still validates the shared request contract.</p><pre className="api-code"><code>{tfidfExample}</code></pre><pre className="api-code"><code>{similarityExample}</code></pre></section>
          <section id="response"><p className="section-number">06</p><h2>Response fields</h2><div className="api-fields"><div><b>tokenCount / wordCount</b><p>Words used as the result denominator after the endpoint&apos;s documented cleanup rule.</p></div><div><b>vocabularySize</b><p>Number of distinct analyzed words.</p></div><div><b>rows / unigrams</b><p>Words with counts, percentages, and per-1,000 values.</p></div><div><b>totalRows / returnedRows / truncated</b><p>Table-size metadata that makes an applied result limit explicit.</p></div><div><b>offset / nextOffset / hasMore</b><p>Stable pagination metadata for complete ordered vocabulary, density, n-gram, or comparison tables. IDF tables use the corresponding <code>idfOffset</code> fields.</p></div><div><b>bigrams / trigrams</b><p>Two- and three-word phrase tables where supported.</p></div><div><b>focusCoverage / trackedKeywords</b><p>Exact phrase counts and occurrences per 1,000 words.</p></div><div><b>fittedExponent</b><p>Estimated frequency-curve exponent returned by the Bag of Words analyzer.</p></div><div><b>tf / idf / tfidf / cosine</b><p>Term and pair-level weighting diagnostics for direct model usage.</p></div></div><p>Successful responses identify <code>apiVersion</code> and storage as <code>none</code>. Errors use a stable <code>error.code</code> and a human-readable <code>error.message</code>. Every response exposes a random <code>X-Request-ID</code> for support correlation.</p></section>
          <section id="limits"><p className="section-number">07</p><h2>Limits and safe use</h2><ul><li>Each raw text field is limited to 500,000 characters and 100,000 analyzable words.</li><li><code>focus</code> accepts at most 100 non-empty, analyzable phrases of no more than 200 characters each.</li><li>Remote response bodies are limited to 2,000,000 bytes under one 12-second DNS, redirect, and download deadline.</li><li>Only public HTTP and HTTPS pages are accepted; connections use the validated public address and revalidate every redirect.</li><li>Compare, similarity, and TF-IDF requests share limits of 2,000,000 pasted characters, 250,000 analyzable words, 5,000,000 remote bytes, and three concurrent downloads.</li><li>Generated result tables return no more than 5,000 rows per page; density tables default to 2,000 rows, while explicitly requested tracked phrases are limited to 100 and are preserved.</li><li>Serialized JSON responses are limited to 5,000,000 bytes.</li><li>The public service allows 30 cost units per minute. Request cost is <code>1 + one unit per source after the first + two units per URL + one unit per full 100,000 pasted-text characters</code>, capped at 30.</li><li>Responses expose rate-limit limit, remaining, reset, and retry headers to browser clients.</li><li>Use the results as editorial diagnostics, not as guarantees of search ranking.</li></ul></section>
          <section id="discovery"><p className="section-number">08</p><h2>Agent discovery and health</h2><p>Agents can read <a href="/llms.txt">/llms.txt</a> for a concise capability map and <a href="/openapi.json">/openapi.json</a> for the callable contract. Both resources and every versioned API endpoint support cross-origin access. Liveness monitors can call <code>GET /api/health</code>; it returns bounded service metadata and a <code>rateLimit</code> state of <code>shared</code>, <code>local</code>, or <code>degraded</code> without testing or exposing submitted content.</p><p>For exact browser, API, CLI, MCP, storage, logging, and analytics boundaries, read <Link href="/privacy">Privacy and Data Handling</Link>.</p></section>
