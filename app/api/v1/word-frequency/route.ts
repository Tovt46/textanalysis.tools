...
export function OPTIONS(request:Request){return observeApiRequest(request,"word-frequency",()=>apiOptions());}

export function GET(request:Request){
  return observeApiRequest(request,"word-frequency",()=>apiJson({apiVersion:API_VERSION,name:"Text Analysis Tools API",operation:"word-frequency",method:"POST",documentation:"/api-docs",openapi:"/openapi.json"}));
}

export function POST(request:Request){
  return observeApiRequest(request,"word-frequency",async()=>{
    ...
    try{
      ...
      const result=limitRows(analyzeWordFrequency(input),parseResultRowLimit(body.limit),parseResultRowOffset(body.offset));
