...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){
  ...
  return localizedToolMetadata("uk",slug);
}

export default async function UkrainianToolPage({params}:{params:Promise<{slug:string}>}){
  const {slug}=await params;
  if(!isLocalizedToolSlug(slug))notFound();
  return <LocalizedToolPage locale="uk" slug={slug}/>;
}
