...
function ZoneGuide({ tolerance }: { tolerance:number }) {
  return <div className="zone-guide">
    <div><ZoneBadge zone="within"/><p><b>В пределах.</b> Частота слова укладывается в выбранный допуск ×{tolerance.toFixed(1)}. По одному этому сигналу ничего менять не нужно.</p></div>
    <div><ZoneBadge zone="above"/><p><b>Выше модели.</b> Слово встречается более чем в ×{tolerance.toFixed(1)} чаще расчётной частоты. Проверьте повторения, но не удаляйте автоматически.</p></div>
    <div><ZoneBadge zone="below"/><p><b>Ниже модели.</b> Слово встречается заметно реже расчётной частоты. Это не означает, что его обязательно нужно добавить.</p></div>
    <div><ZoneBadge zone="sparse-tail"/><p><b>Редкий хвост.</b> Модель ожидает меньше одного вхождения. На этой части списка данных недостаточно для редакторского вывода.</p></div>
  </div>;
}
  ...
  return <main>
    ...
    <form className="workspace" onSubmit={analyze}>
      <section className="input-card">
        ...
        <label className="field wide"><span>Контрольные фразы <Tip>Слова и фразы, которые вы хотите проверить отдельно. Инструмент покажет точное число упоминаний; это не список обязательных SEO-ключей.</Tip></span><input value={focus} onChange={e=>setFocus(e.target.value)} placeholder="online tarot, relationships, feelings"/><small>Введите через запятую. Мы посчитаем точные упоминания каждой фразы.</small></label>
      </section>
      <aside className="settings-card">
        <div className="section-head simple"><div><span>02</span><h2>Настройки</h2></div></div>
        <label className="field"><span>Язык <Tip>Определяет, какой список стоп-слов применяется. При выборе языка редактор стоп-слов переключается вместе с ним.</Tip></span><select value={language} onChange={e=>changeLanguage(e.target.value)}><option value="en">English</option><option value="ru">Русский</option><option value="uk">Украинский</option><option value="auto">Определить автоматически</option></select><small>{language==="auto"?"После анализа откроется список определённого языка.":`Редактор стоп-слов сейчас использует список ${language.toUpperCase()}.`}</small></label>
