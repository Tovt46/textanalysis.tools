...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){
  ...
  return localizedInfoMetadata("es",slug);
}

export default async function SpanishInfoPage({params}:{params:Promise<{slug:string}>}){
  const {slug}=await params;
  if(!isLocalizedInfoSlug(slug))notFound();
  return <LocalizedInfoPage locale="es" slug={slug}/>;
}
