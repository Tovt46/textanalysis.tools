...
export default function WordFrequencyTool({uiLang="en"}:{uiLang?:UiLang}){
  ...
  function changeLanguage(value:"auto"|TextLanguage){cancel();setLanguage(value);if(value!=="auto")setEditorLanguage(value);trackEvent("language_changed",{tool:"word_frequency_counter",text_language:value});}
  function changeEditorLanguage(value:TextLanguage){cancel();setEditorLanguage(value);setLanguage(value);trackEvent("language_changed",{tool:"word_frequency_counter",text_language:value,control:"stopword_editor"});}
  ...
  return <>
    ...
    {result&&<section className="frequency-results" id="frequency-results">
      <div className="results-title"><div><span>03</span><h2>{copy.results}</h2></div><div className="results-actions"><p>{copy.detected}: <b>{result.language.toUpperCase()}</b></p><CopyResultAction tool="word-frequency" locale={uiLang} value={result}/></div></div>
